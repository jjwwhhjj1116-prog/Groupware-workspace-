...
export const Header = () => {
  ...
  return (
    <header className="bg-[var(--color-surface)] border-b border-[var(--color-border)] h-14 flex min-w-0 items-center justify-end overflow-hidden px-2 sm:px-4 lg:justify-between xl:px-6 z-40 sticky top-0">
      <div className="hidden min-w-0 items-center gap-3 lg:flex">
        <h2 className="min-w-0 truncate text-[15px] font-semibold text-[var(--color-text-main)]">
        ...
        {currentUser && ['SUPER_ADMIN', 'SYSTEM_ADMIN'].includes(currentUser.role) && (
          <div className="hidden bg-gray-100/80 rounded p-1 border border-[var(--color-border)] gap-1 ml-2 xl:flex">
            <button
        ...
        <div className="ml-1 hidden px-2 py-1 bg-[var(--color-bg)] text-[var(--color-text-sub)] text-[11px] font-semibold rounded border border-[var(--color-border)] 2xl:flex items-center gap-1.5">
      ...
      <div className="flex min-w-0 items-center gap-2 sm:gap-3 xl:gap-5">
        {/* Language Toggle */}
        <div className="flex shrink-0 items-center bg-[var(--color-bg-sub)] rounded px-0.5 py-0.5 text-[10px] font-bold border border-[var(--color-border)]">
          <button
        ...
        {/* Mock Login Switcher */}
        <select
          className="min-w-0 max-w-[118px] border border-[var(--color-border)] rounded-md px-2 py-1 text-xs bg-[var(--color-bg)] text-[var(--color-text-sub)] outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)] focus:border-indigo-500 sm:max-w-[190px] xl:max-w-[260px]"
        ...
        <div className="hidden items-center gap-4 border-l border-[var(--color-border)] pl-4 md:flex">
        ...
        <Link href="/settings" className="flex items-center gap-2 cursor-pointer hover:bg-[var(--color-bg)] px-2 py-1.5 rounded-md transition-colors ml-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]">
          ...
          <span className="hidden text-[13px] font-semibold text-[var(--color-text-main)] 2xl:inline">{getUserDisplayName(currentUser)}</span>
        </Link>
      </div>
    </header>
  );
